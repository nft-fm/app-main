...
const MusicPlayer = (props) => {
  ...
  const startSong = (_songFile) => {
    ...
    audioContextRef.current.decodeAudioData(abSong, (_buffer) => {
      ...
      let index = props.nfts.indexOf(selectedNft);
  ...
  useEffect(() => {
    ...
    console.log("NEXT BUFFER", props.nextBuffer);
    console.log("PREV BUFFER", props.prevBuffer);
    console.log("STATUS", props.status)
    if (props.nextBuffer && props.status === "NEXT") {
      let next = props.nextBuffer;
      startSong(next);
      props.setStatus();
      props.fetchNextBuffer();
    } 
    else if (props.prevBuffer && props.status === "PREV") {
      let prev = props.prevBuffer;
      startSong(prev);
      props.setStatus();
      props.fetchPrevBuffer();
    }
