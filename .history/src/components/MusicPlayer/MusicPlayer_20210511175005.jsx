...
const MusicPlayer = (props) => {
  ...
  const startPartialSong = async (songFile) => {
    ...
    audioContextRef.current.decodeAudioData(abSong, async (_buffer) => {
      ...
      await axios.post("api/nft-type/getSong", { key: nft.address + "/" + nft.audioUrl.split('/').slice(-1)[0] })
            .then((fullFile) => {
            startRemainingSong(fullFile, _bufferSrc);
  ...
  const startRemainingSong = async (songFile, partialBufferSrc) => {
  ...
  useEffect(() => {
    ...
    if (isPlaying) {
      intervalId = setInterval(() => {
        ...
        console.log("currentTime", bufferSrc.context.currentTime);
        console.log("duration", bufferSrc.context.duration);
        if (time >= dur  || bufferSrc.context.currentTime >= bufferSrc.context.duration) {
          time = dur;
          stopSong();
          setNextNft();
