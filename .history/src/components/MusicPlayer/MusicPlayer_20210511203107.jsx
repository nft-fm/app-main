...
const MusicPlayer = (props) => {
  ...
  const startRemainingSong = () => {
    if (bufferSrc.context.currentTime >= bufferSrc.buffer.duration) {
        console.log("STARTING REMAINING SONG", bufferSrc.context.state);
        ...
        setDur(_fullBufferSrc.buffer.duration);
        setBufferSrc(_fullBufferSrc);
        setFullBufferSrc(false);
        setIsPlaying(true);
        setIsLoading(false);
    }
  }
  ...
  useEffect(() => {
    let intervalId;
    console.log("_______")
    console.log("songFullyLoaded", songFullyLoaded);
    console.log("bufferSrc", bufferSrc);
    console.log("fullBufferSrc", fullBufferSrc);
    if (bufferSrc) console.log("buffer src current time", bu)
    console.log("_______")
    if (isPlaying) {
      intervalId = setInterval(() => {
        let time = bufferSrc.context.currentTime - startTime;
        //If partial song is finished
        if (!songFullyLoaded && bufferSrc.context.currentTime >= bufferSrc.buffer.duration) {
          if (!fullBufferSrc && bufferSrc.context.state === "running") stopSong();
          else if (fullBufferSrc.context.state !== "running") startRemainingSong();
        }
        else if (time >= dur) {
          time = dur;
          stopSong();
    ...
    else if (!songFullyLoaded && bufferSrc &&
            fullBufferSrc && fullBufferSrc.context &&
            fullBufferSrc.context.state !== "running" &&
            bufferSrc.context.currentTime >= bufferSrc.buffer.duration && fullBufferSrc) {
      startRemainingSong();
    }
