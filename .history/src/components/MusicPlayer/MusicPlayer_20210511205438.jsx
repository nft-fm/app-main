...
const MusicPlayer = (props) => {
  ...
  const startRemainingSong = () => {
    if (bufferSrc.context.currentTime >= bufferSrc.buffer.duration) {
        ...
        setBuffer(_fullBufferSrc.buffer);
  ...
  const prepareRemainingSong = async (songFile, partialBufferSrc) => {
    ...
    audioContextRef.current.decodeAudioData(abSong, async (_buffer) => {
      ...
      if (partialBufferSrc.context.currentTime >= partialBufferSrc.buffer.duration) {
        ...
        console.log("SET FULL BUFFER SRC")
  ...
  useEffect(() => {
    let intervalId;
    if (fullBufferSrc) console.log("full buffer current state", fullBufferSrc.context.state)
    if (isPlaying) {
      console.log("")
      intervalId = setInterval(() => {
        ...
        if (!songFullyLoaded && bufferSrc.context.currentTime >= bufferSrc.buffer.duration) {
          if (!fullBufferSrc && bufferSrc.context.state === "running") {
            console.log("STOPPING SONG")
            stopSong();
          }
          else if (fullBufferSrc.context.state !== "running") {
            console.log("IF TO REMAINING SONG")
            startRemainingSong();
          }
        }
        else if (time >= dur) {
      ...
      console.log("buffer loaded after end of preview")
      startRemainingSong();
    }
