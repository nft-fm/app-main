...
export const PlaylistProvider = ({ children }) => {
  ...
  const fetchPrevNext = async () => {
    ...
    let _nfts = [...nfts];
    const next_nft = nfts[nextIndex];
    const prev_nft = nfts[prevIndex];
    if (!nft.buffer) {
      const nextSong = await axios.post("api/nft-type/getSong", { key: nft.address + "/" + nft.audioUrl.split('/').slice(-1)[0] })
    }
    _nfts[nextIndex] = {..._nfts[newIndex],
      buffer: songFile}
  }
