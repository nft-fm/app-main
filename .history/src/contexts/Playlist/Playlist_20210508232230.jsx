...
export const PlaylistProvider = ({ children }) => {
  ...
  const fetchPrevNext = async () => {
    ...
    if (!next_nft.buffer) {
      let next_song = await axios.post("api/nft-type/getSong", { key: nft.address + "/" + nft.audioUrl.split('/').slice(-1)[0] })
      _nfts[nextIndex] = {..._nfts[nextIndex],
        buffer: nextSong};
    }
    else if (!prev_nft.buffer) {
      let prev_song = await axios.post("api/nft-type/getSong", { key: nft.address + "/" + nft.audioUrl.split('/').slice(-1)[0] })
      _nfts[nextIndex] = {..._nfts[nextIndex],
        buffer: nextSong};
    }
