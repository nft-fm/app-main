...
export const PlaylistProvider = ({ children }) => {
  ...
  const fetchPrevNext = async () => {
    ...
    if (!next_nft.buffer) {
      const next_song = await axios.post("api/nft-type/getSong", { key: next_nft.address + "/" + nft.audioUrl.split('/').slice(-1)[0] })
      _nfts[nextIndex] = {..._nfts[nextIndex],
        buffer: next_song};
    }
    else if (!prev_nft.buffer) {
      const prev_song = await axios.post("api/nft-type/getSong", { key: prev_nft.address + "/" + nft.audioUrl.split('/').slice(-1)[0] })
      _nfts[nextIndex] = {..._nfts[nextIndex],
                          buffer: prev_song};
    }
    setNfts(_nfts);
