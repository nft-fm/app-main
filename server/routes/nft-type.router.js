...
router.post('/new', web3RejectUnauthenticated, async (req, res) => {
  try {
    const { contractAddress, picture, rarity, stats, mintLimit, name } = req.body;

    const newNftType = new NftType({ contractAddress, picture, rarity, stats, mintLimit, name })
...
router.get('/', async (req, res) => {
  try {
    let nftType = await NftType.findOne({ contractAddress: req.body.contractAddress });
    res.send(nftType);
  } catch (error) {
    console.log(error);
    res.status(500).send("server error")
