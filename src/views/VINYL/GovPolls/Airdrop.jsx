...
const Airdrop = () => {
  ...
  const parseHolders = async () => {
    await axios
      .post("/api/airdrop/getHoldersFromDB", { batch: parseBatch })
      .then((res) => {
        let arr = [];
        ...
        arr.map(({ address, quantity }) => {
          ...
          const toAirdrop = roundDown(
            (quantity / totalNftsOwned) * quantityToAirdrop
          ).toFixed(9);
  ...
  const pullHoldersFromOpensea = async () => {
    ...
    swal
      .fire({
        title:
          "this will create a new batch at this time by pulling from the opensea API",
        showDenyButton: "true",
      })
      .then(async (result) => {
        if (result.isConfirmed) {
          ...
          for (let index = 0; index < nftIds.length; index++) {
            axios
              .post("/api/airdrop/getHoldersFromOpenSea", {
                nftId: index,
                batch: nextBatchNum,
              })
              .then((res) => {
  ...
  return (
    <Container>
      {nftIds && nextBatchNum && (
        <Contained>
          <div>Next Batch (to fetch): {nextBatchNum}</div>
